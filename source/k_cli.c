...
static char cmd_str4[MAX_LEN_CMD_STR];
static int argc;
    ...
    switch (idx_getCmd)
    {
        ...
        case 0x10:
            ...
            if (uart_getch(4,&ch)) //receive
            {
                ...
                if (idx_cmdStr < MAX_LEN_CMD_STR-3)
                {
                    if ( (ch==0x0D))
                    {
                        cmd_str4[idx_cmdStr++] = 0x0D;
                        cmd_str4[idx_cmdStr++] = 0x0A;
                        cmd_str4[idx_cmdStr++] = 0;

                        if (cmd_str4[1]==0xa && cmd_str4[2]==0)
                        {
                            idx_getCmd = 0x99;
                        }
                        else
                        {
                            ...
                            //PRINTLINE;
                            idx_getCmd = 0x20;
                            //debugprintf("get_cmd4: %s\r\n", cmd_str4);
                            // wifiprintf("get_cmd4: %s\r\n", cmd_str4);
                            //wifistring(cmd_str4);
                        }
                    }
                    else if ( (ch== '#') )
                ...
                else
                {
                    ...
                    // debugstring("input string is over MAX_LEN_CMD_STR.\r\n");
                    // debugstring("the string is discarded.\r\n");
                    wifistring("input string is over MAX_LEN_CMD_STR.\r\n");
                    wifistring("the string is discarded.\r\n");
                }
            }
            break;
        ...
        case 0x20:
            for (i=0; i<cmd_cnt; i++)
            {
                ...
                if (strncmp(cmd_str4, cmd_registry[i].cmd, strlen(cmd_registry[i].cmd)) == 0)
                {
                    //get_argv(cmd_str4);
                    //PRINTLINE;
                    get_args(cmd_str4);
