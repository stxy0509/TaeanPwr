...
SURFACE* bitfont_makesurface( EGL_FONT* pFont, char* str )
{
    int i = 0;
    int x = 0;
    int index;
    ...
    int w ;
    SURFACE* surf ;
    cur_fontdata = ( BITFONTDATA* )pFont->ext;
    if( engchar_surf == 0 )
    {
        return 0 ;
    }
    ...
    surf->pal->colorkey = 0;
    surf->pal->colors[1] = cur_fontdata->fontclr;
    while( str[i] )
    {
        low = str[i];
        i++;
...
static BOOL bf_init()
{
    ...
    engchar_surf_vert = CREATE_SURFACE( ENG_FONT_H, ENG_FONT_W, 8 );
